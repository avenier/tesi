...
\chapter{Il Progetto}\label{introduzione}
	...
	\section{Studio delle applicazioni CRM}
		...
		\begin{enumerate}
			\item viene creata un entità \textbf{Account} per un'azienda cliente o che potrebbe potenzialmente diventarlo, nella quale vengono censiti tutti i dati che si hanno a disposizione (nome, sito web, email, ricavi annuali, numero di dipendenti, area commerciale,...);
			\item vengono create una o più entità \textbf{contatto} associate agl'\textbf{Account}, queste contengono tutti i dati relativi alle persone dell'azienda cliente alle quali proporre offerte commerciali.
			\item ogni \textbf{Prodotto} che si intende vendere deve essere inserito nel CRM  con un apposita descrizione ed un prezzo di listino associato;
			\item nel momento in cui si vuole proporre un offerta commerciale ad un cliente, deve essere creata un entità \textbf{Offerta} alla quale associare una serie di prodotti (scontabili rispetto al prezzo di listino);
			\item approvata l'offerta da parte dal cliente, a questa può essere associato un \textbf{Ordine}, contenente tutti i dati di spedizione, fatturazione e molti altri.
		\end{enumerate}
		...
		\paragraph{Le entità di dati}
			Attraverso lo studio si SalesForce e di ADProject sono emerse le entità di dati che sarebbe stato utile importare dal CRM:
			\begin{itemize}
				...
				\item \textbf{Prodotti:} i prodotti corrispondono con l'output principale ottenuto portando a termine un progetto
				...
				\item \textbf{Utenti:} il personale aziendale che utilizza il CRM (probabilmente il reparto commerciale) è bene che sia annoverato tra le risorse aziendali
			\end{itemize}
		\par
		\subsection{Procedure di Autenticazione}
			Per avere l'accesso ai dati del CRM, l'applicazione ADCrm deve potersi autenticare presso i server di SalesForce e Dynamics. Fortunatamente entrambe le applicazioni permettono l'autenticazione attraverso protocollo OAuth2.0.
			\paragraph{OAuth2.0}
			...
			Questo \glo{token} è un una serie di caratteri alfanumerici da allegare all'\glo{header} della richiesta http, senza il quale non si potrebbero recuperare i dati richiesti al CRM.
				\begin{figure}[!hb]
					\centering
			...
			\begin{enumerate}
				...
				\item nel caso in cui la richiesta sia stata effettuata con successo, il server risponde con un \glo{JSON} contenente i \glo{token} necessari ad interrogare il CRM;
		...
		\subsection{Simulazione query}
			Si è presto reso necessario verificare che il processo di autenticazione funzionasse correttamente e che fosse possibile recuperare tutti i dati voluti attraverso le \glo{query}, senza tuttavia cominciare la fase di codifica di ADCrm (dato che modifiche in corso d'opera all'architettura dell'applicazione, avrebbero causato un notevole spreco di tempo e sforzi).
			Per venire in contro a questa esigenza si è deciso di simulare tutte le chiamate che il sistema avrebbe effettuato tramite \textit{Postman}.
			\paragraph{Postman Rest Client} è un applicazione che permette di comporre richieste http in maniera semplice e veloce, risultando particolarmente utile per testare il funzionamento di API proprie e altrui.
			\par
			...
			\begin{figure}[!hb]
				...
				\label{fig:postman}
			\end{figure}
			Una volta simulate tutte le richieste è stato possibile iniziare la fase di progettazione avendo ben chiari i passaggi fondamentali che ADCrm avrebbe dovuto compiere.
