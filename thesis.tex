...
\chapter{Il Progetto}\label{introduzione}
	\section{Piano di Lavoro}
		Il progetto, per quanto stabilito nel piano di lavoro dello stage, comporta l'integrazione di \textit{Salesforce} con \textit{ADProject} ma, dato che la parte principale dell'applicazione è stata ultimata con due settimane di anticipo rispetto al previsto, si è deciso di integrare nel servizio \textit{\textbf{ADCrm}} anche un secondo CRM: \textit{\textbf{Microsoft Dynamics}}.
		Nei successivi capitoli verrà descritta l'integrazione con entrambi i sistemi e verranno spiegate le sfide riscontrate in questo progetto, tuttavia per evitare eccessive ripetizioni verrà posta particolare attenzione alle parti riguardanti \textit{\textbf{SalesForce}}.
	\section{Studio delle applicazioni CRM}
		...
		\begin{enumerate}
			...
			\item vengono create una o più entità \textbf{Contatto} associate ad ogni \textbf{Account}, queste contengono tutti i dati relativi alle persone dell'azienda cliente, alle quali proporre offerte commerciali;
		...
		\paragraph{Le classi di dati}~\\
			Attraverso lo studio di SalesForce e di ADProject sono emerse le classi di dati che sarebbero state utili importare dal CRM:
			\begin{itemize}
